...
describe("Given I'm connected as an employee on the NewBill page", () => {
  ...
  beforeEach(() => {
    ...
    window.localStorage.setItem(
      "user",
      JSON.stringify({ type: "Employee", email: "employee@test.fr" })
    );
    newBill = new NewBill({
      document,
      onNavigate: jest.fn(),
  ...
  describe("When I upload a file that is .jpg .jpeg or .png", () => {
    ...
    test("Then the form should contain the file and create a new bill", () => {
      // Mock the store.bills().create function to return a Promise
      const createMock = jest
        .fn()
        .mockResolvedValue({ fileUrl: "test", key: "test" });
      const store = {
        bills: jest.fn(() => ({
          create: createMock,
        })),
      ...
      // Check if store.bills().create was called with the correct arguments
